...
public class FrmXyLyDatPhong extends JFrame implements Runnable, ActionListener, MouseListener {
	...
	private ImageIcon icon_PhongTrong = new ImageIcon(getClass().getResource("/img/room-mini-3.png"));
	private ImageIcon icon_PhongCho = new ImageIcon(getClass().getResource("/img/room-mini-2.png"));
	private ImageIcon icon_PhongDSD = new ImageIcon(getClass().getResource("/img/room-mini-1.png"));
	private ImageIcon icon_PhongVip = new ImageIcon(getClass().getResource("/img/vip-Mini.gif"));
	 ...
	 * Create the frame.
	 */
	public FrmXyLyDatPhong(String nhanVien) {
		setTitle("Đặt phòng");
		...
		time_VaoPhong.getCalendarButton()
				.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/icons8-calendar-50.png")));
...
//						thongBaoLoi("Giờ ra phải sau giờ vào", "Nhập thời gian vào");
		...
		lbl_ThoiGianDat_8.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/icons8-stop-button-32.png")));
		...
		lbl_ThoiGianDat_8_1.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/vip-Mini.gif")));
		...
		btn_ChonPhong.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/icons8-down-60.png")));
				...
				if (hanhDong.equals(""))
					...
					if (hanhDong.equals("Đặt phòng")) {
						...
						if (hoiLai == JOptionPane.YES_OPTION) {
							...
							FrmXuLyPhongDangSuDung frm = new FrmXuLyPhongDangSuDung(maNhanVienTao,
					...
					if (hanhDong.equals("Tạo phiếu đặt")) {
						...
						if (hoiLai == JOptionPane.YES_OPTION) {
							// chuyển sang trang phòng đang sử dụng
							FrmXyLyPhieuDatPhong frm = new FrmXyLyPhieuDatPhong(maNhanVienTao,
									khachDuocChon.getSoDienThoai());
							FrmTrangChu.jtab_NoiDung.remove(FrmTrangChu.jtab_NoiDung.getSelectedComponent());
							FrmTrangChu.jtab_NoiDung.add(frm.pnBorder);
						}
		...
		btn_ThemKH.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/DanhMuc_Them.png")));
		...
		btn_LocDanhSach.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/Search-icon.png")));
...
//			butt_danhSachPhong[viTriNutDuocChon].setBorder(null);
	...
	public void setIcon(int viTri) {
		...
		if (dsDLHienTai.get(viTri).getLoaiPhong().getTenLoaiPhong().equals("Vip")) {
			iconVip = new JLabel();
			iconVip.setIcon(new ImageIcon(FrmXyLyDatPhong.class.getResource("/img/VIP.gif")));
...
//							+ " phút", "Xung đột với mã " + i);
	...
	public int kiemTraThoiGian(Phong phong) {
		...
		if (timeVaoMoi.getHour() < 8) {
			thongBaoLoi("Quán 8 giờ mới bắt đầu mở cửa!\n Giờ vào phòng phải >8", "Lỗi nhập thời gian đặt");
			return -1;
		}
	...
	private ArrayList<Phong> locDanhSach() {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj.equals(btn_LocDanhSach)) {
			...
			ArrayList<Phong> dsLoc = locDanhSach();
			// lọc ds
		...
		if (obj.equals(btn_ChonPhong)) {
			...
			if (btn_ChonPhong.getText().equals("Chọn phòng")) {
				...
				Phong p = dao_Phong.getPhongTheoMa(maPhongDuocChon);
	...
	public boolean taoPhieuDatPhongMoi() {
