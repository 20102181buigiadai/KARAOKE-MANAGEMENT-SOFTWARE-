...
public class FrmTimKiemDichVu extends JFrame implements KeyListener, ActionListener {
	...
	private ArrayList<DichVu> dsTimDuoc = dao_DichVu.getAllDichVu();

	public FrmTimKiemDichVu(String maNhanVien) {
	...
	private void createUI(String maNhanVien) {
		...
		if (maNhanVien.equals("admin") || nVien.getLoaiNhanVien().getMaLoai().equals("LNV001")
				|| nVien.getLoaiNhanVien().getMaLoai().equals("LNV002"))
			pnlBorder.add(pnlButton, BorderLayout.SOUTH);
		this.add(pnlBorder);

		loadData(dao_DichVu.getAllDichVu());
	...
	public void keyReleased(KeyEvent e) {
		dsTimDuoc = dao_DichVu.search(txtTimKiem.getText().trim());
		loadData(dsTimDuoc);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnCapNhatDichVu)) {
			FrmQuanLyDichVu frmQuanLyDichVu = new FrmQuanLyDichVu(dsTimDuoc);
