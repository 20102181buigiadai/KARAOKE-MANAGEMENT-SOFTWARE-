...
public class FrmXuLyThanhToan extends JFrame implements ActionListener {
	...
	static ArrayList<KhachHang> dsALL_KH = dao_KhachHang.getALLKhachHang();
	...
	private void createUI(ArrayList<HoaDon> dsHD, NhanVien nhanVienTao) {
		// Font
	...
	private void loadDL(ArrayList<HoaDon> dsHD, NhanVien nhanVienTao) {
		dsHoaDon = dsHD;
		...
		lblGiaTriGioRa.setText(dtf2.format(thoiGianHT));
		...
		for (HoaDon hd : dsHoaDon) {
			for (ChiTietHoaDon_Phong cthd_P : dsALL_CTHD_Phong)
				if (cthd_P.getHoaDon().getMaHoaDon().equals(hd.getMaHoaDon())) {
					...
					if (gioVao.isAfter(cthd_P.getGioVao()))
	...
	public void docDSPhong() {
		...
		for (int i = 0; i < dscthd_Phong.size(); i++) {
			if (dscthd_Phong.get(i).getGioRa() == null)
				dscthd_Phong.get(i).setGioRa(thoiGianHT);
